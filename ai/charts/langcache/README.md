...
| Cache database registry | One or more Redis instances with the search module | Where cache entries and their vectors actually live. Registered by ID in the `controlplane.secrets` overlay only — the Data Plane has no database registry of its own; it resolves each cache's Redis target from the `databaseUrls` the Control Plane already persisted at creation time. |
...
Metadata Redis URLs, the Control Plane's cache database registry, and (when
`dataplane.embedding.credentials.type=static`) the Data Plane's embedding
credential never go in `values.yaml` or the rendered ConfigMap. Each of the
Data Plane and Control Plane reads its own pre-created overlay Secret,
deep-merged over its rendered config by the loader at runtime:
...
`dp-overlay.yaml` (no database registry — the Data Plane resolves each
cache's Redis target from the `databaseUrls` the Control Plane already
persisted at creation time, not from a registry of its own):
...
receives one — but it does own the database registry):
...
- refuses `identityService.external.baseURL` unless it is `https://`.
  ...
  outside `fips`, but is not honored under `fips` — a chart labeled `fips`
  never renders a plaintext introspection endpoint, full stop.
