...
echo "=== Case 9b: security.profile=fips + allowInsecureTransport=true still refuses a plaintext external Identity Service ==="
render_expect_fail --set identityService.mode=external \
  --set identityService.external.baseURL=http://suite-ids.example.com \
  --set identityService.external.credential.existingSecret=ext-ids-cred \
  --set identityService.external.allowInsecureTransport=true \
  --set security.profile=fips
echo "OK: fips is a hard guarantee — allowInsecureTransport is not honored under fips"
