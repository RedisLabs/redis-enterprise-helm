...
metadata.urls and (when embedding.credentials.type=static)
embedding.credentials.api_key — no database registry; the Data Plane
resolves each cache's Redis target from its own persisted databaseUrls.
*/}}
...
(metadata.urls, embedding.credentials.api_key) are never present here — they
arrive via the mounted overlay Secrets. No database registry either: the
Data Plane resolves each cache's Redis target from its own persisted
databaseUrls (see langcache.dataplaneOverlaySecretNames above).
*/}}
...
{{- fail "dataplane.secrets.secretName is required: a pre-created Secret holding an overlay with metadata.urls (and embedding.credentials.api_key when dataplane.embedding.credentials.type=static)" -}}
{{- end -}}
...
{{- end -}}
{{- if and .Values.security (eq (default "" .Values.security.profile) "fips") (not (hasPrefix "https://" $ext.baseURL)) -}}
{{/*
  Unlike the non-fips default (where allowInsecureTransport is a real,
  supported opt-out), fips must be a hard guarantee: honoring
  allowInsecureTransport here would let a chart labeled "fips" render
  GODEBUG=fips140=on while actually introspecting over plaintext HTTP,
  silently defeating the posture it claims to have.
*/}}
{{- fail "security.profile=fips requires identityService.external.baseURL to be an https:// URL; allowInsecureTransport is not honored under fips" -}}
{{- end -}}
